...
void Preprocess::velodyne_handler(const sensor_msgs::msg::PointCloud2::UniquePtr &msg)
{
  ...
  // Check if time field has valid range (should span ~100ms for 10Hz scan)
  // If time range is too small, use angle-based estimation instead
  float time_first = pl_orig.points[0].time;
  float time_last_pt = pl_orig.points[plsize - 1].time;
  float time_range = std::abs(time_last_pt - time_first);

  // time_range should be ~0.1s for 10Hz, require at least 0.01s (10ms) to be valid
  if (time_range * time_unit_scale > 10.0)  // 10ms minimum range
  {
  ...
  else
  {
    ...
    if (!warned) {
      std::cerr << "[WARN] Point cloud time range too small (" << time_range * time_unit_scale
                << "ms), using angle-based time estimation" << std::endl;
      warned = true;
    }
  ...
  if (feature_enabled)
  {
    ...
    for (int i = 0; i < plsize; i++)
    {
      ...
      added_pt.z = pl_orig.points[i].z;
      added_pt.intensity = pl_orig.points[i].intensity;
      // Subtract first point time to get relative offset (0 ~ scan_duration)
      // This fixes the issue where rslidar_sdk adds +0.1s bias to time field
      added_pt.curvature = (pl_orig.points[i].time - time_first) * time_unit_scale;  // units: ms
  ...
  else
  {
    for (int i = 0; i < plsize; i++)
    {
      ...
      // This fixes the issue where rslidar_sdk adds +0.1s bias to time field
      added_pt.curvature =
          (pl_orig.points[i].time - time_first) * time_unit_scale;  // curvature unit: ms
