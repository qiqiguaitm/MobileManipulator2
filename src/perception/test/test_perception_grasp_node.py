...
def _ros2_available() -> bool:
    """检查 ROS2 环境是否可用"""
    try:
        from perception.msg import GraspObject
        return True
    except ImportError:
        return False
...
class TestImports:
    ...
    @pytest.mark.skipif(
        not _ros2_available(),
        reason='需要 ROS2 环境和已编译的 perception 消息包'
    )
    def test_import_msg(self):
        """测试消息导入"""
        from perception.msg import GraspObject, GraspObjectArray
        assert GraspObject is not None
        assert GraspObjectArray is not None
