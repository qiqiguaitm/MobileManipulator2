...
struct neighborsearch_kernel {
  neighborsearch_kernel(int k, const thrust::device_vector<Eigen::Vector3f>& target, thrust::device_vector<FloatIntPair>& k_neighbors)
  ...
  __host__ __device__ void operator()(const Tuple& idx_x) const {
    ...
    FloatIntPair* k_neighbors = thrust::raw_pointer_cast(k_neighbors_ptr) + idx * k;

    // priority queue using custom compare
    struct compare_type {
      __host__ __device__ bool operator()(const FloatIntPair& lhs, const FloatIntPair& rhs) {
        return lhs.first < rhs.first;
      }
    };

    typedef nvbio::vector_view<FloatIntPair*> vector_type;
    typedef nvbio::priority_queue<FloatIntPair, vector_type, compare_type> queue_type;
    queue_type queue(vector_type(0, k_neighbors - 1));

    for(int i = 0; i < k; i++) {
      float sq_dist = (pts[i] - x).squaredNorm();
      queue.push(FloatIntPair(sq_dist, i));
    }
  ...
  thrust::device_ptr<FloatIntPair> k_neighbors_ptr;
};

struct sorting_kernel {
  sorting_kernel(int k, thrust::device_vector<FloatIntPair>& k_neighbors) : k(k), k_neighbors_ptr(k_neighbors.data()) {}
...
void brute_force_knn_search(const thrust::device_vector<Eigen::Vector3f>& source, const thrust::device_vector<Eigen::Vector3f>& target, int k, thrust::device_vector<FloatIntPair>& k_neighbors, bool do_sort) {
  ...
  k_neighbors.resize(source.size() * k);

  // Initialize with FloatIntPair(-1.0f, -1) using thrust::fill
  thrust::fill(k_neighbors.begin(), k_neighbors.end(), FloatIntPair(-1.0f, -1));
