...
void FastVGICPCudaCore::set_neighbor_search_method(fast_gicp::NeighborSearchMethod method, double radius) {
  ...
  offsets->resize(h_offsets.size());
  thrust::copy(h_offsets.begin(), h_offsets.end(), offsets->begin());
}
...
void FastVGICPCudaCore::set_source_cloud(const std::vector<Eigen::Vector3f, Eigen::aligned_allocator<Eigen::Vector3f>>& cloud) {
  ...
  source_points->resize(points.size());
  thrust::copy(points.begin(), points.end(), source_points->begin());
}
...
void FastVGICPCudaCore::set_target_cloud(const std::vector<Eigen::Vector3f, Eigen::aligned_allocator<Eigen::Vector3f>>& cloud) {
  ...
  target_points->resize(points.size());
  thrust::copy(points.begin(), points.end(), target_points->begin());
}
...
void FastVGICPCudaCore::set_source_neighbors(int k, const std::vector<int>& neighbors) {
  ...
  thrust::copy(k_neighbors.begin(), k_neighbors.end(), source_neighbors->begin());
}
...
void FastVGICPCudaCore::set_target_neighbors(int k, const std::vector<int>& neighbors) {
  ...
  thrust::copy(k_neighbors.begin(), k_neighbors.end(), target_neighbors->begin());
}

// Use custom FloatIntPair instead of thrust::pair
struct untie_pair_second {
  __device__ int operator()(const FloatIntPair& p) const {
    return p.second;
  }
};
...
void FastVGICPCudaCore::get_voxel_correspondences(std::vector<std::pair<int, int>>& correspondences) const {
  // Copy from device to host using custom IntPair
  thrust::host_vector<IntPair> corrs(voxel_correspondences->size());
  thrust::copy(voxel_correspondences->begin(), voxel_correspondences->end(), corrs.begin());
  correspondences.resize(corrs.size());
  std::transform(corrs.begin(), corrs.end(), correspondences.begin(),
    [](const IntPair& x) { return std::make_pair(x.first, x.second); });
}

void FastVGICPCudaCore::get_voxel_num_points(std::vector<int>& num_points) const {
  thrust::host_vector<int> voxel_num_points(voxelmap->num_points.size());
  thrust::copy(voxelmap->num_points.begin(), voxelmap->num_points.end(), voxel_num_points.begin());
...
void FastVGICPCudaCore::get_voxel_means(std::vector<Eigen::Vector3f, Eigen::aligned_allocator<Eigen::Vector3f>>& means) const {
  thrust::host_vector<Eigen::Vector3f, Eigen::aligned_allocator<Eigen::Vector3f>> voxel_means(voxelmap->voxel_means.size());
  thrust::copy(voxelmap->voxel_means.begin(), voxelmap->voxel_means.end(), voxel_means.begin());
...
void FastVGICPCudaCore::get_voxel_covs(std::vector<Eigen::Matrix3f, Eigen::aligned_allocator<Eigen::Matrix3f>>& covs) const {
  thrust::host_vector<Eigen::Matrix3f, Eigen::aligned_allocator<Eigen::Matrix3f>> voxel_covs(voxelmap->voxel_covs.size());
  thrust::copy(voxelmap->voxel_covs.begin(), voxelmap->voxel_covs.end(), voxel_covs.begin());
...
void FastVGICPCudaCore::get_source_covariances(std::vector<Eigen::Matrix3f, Eigen::aligned_allocator<Eigen::Matrix3f>>& covs) const {
  thrust::host_vector<Eigen::Matrix3f, Eigen::aligned_allocator<Eigen::Matrix3f>> c(source_covariances->size());
  thrust::copy(source_covariances->begin(), source_covariances->end(), c.begin());
  covs.resize(c.size());
...
void FastVGICPCudaCore::get_target_covariances(std::vector<Eigen::Matrix3f, Eigen::aligned_allocator<Eigen::Matrix3f>>& covs) const {
  thrust::host_vector<Eigen::Matrix3f, Eigen::aligned_allocator<Eigen::Matrix3f>> c(target_covariances->size());
  thrust::copy(target_covariances->begin(), target_covariances->end(), c.begin());
  covs.resize(c.size());
  std::copy(c.begin(), c.end(), covs.begin());
}
...
void FastVGICPCudaCore::update_correspondences(const Eigen::Isometry3d& trans) {
  thrust::device_vector<Eigen::Isometry3f> trans_vec(1);
  trans_vec[0] = trans.cast<float>();

  if(voxel_correspondences == nullptr) {
    voxel_correspondences.reset(new Correspondences());
  }
  linearized_x = trans.cast<float>();
  find_voxel_correspondences(*source_points, *voxelmap, trans_vec.data(), *offsets, *voxel_correspondences);
}
...
double FastVGICPCudaCore::compute_error(const Eigen::Isometry3d& trans, Eigen::Matrix<double, 6, 6>* H, Eigen::Matrix<double, 6, 1>* b) const {
  thrust::host_vector<Eigen::Isometry3f, Eigen::aligned_allocator<Eigen::Isometry3f>> trans_host(2);
  trans_host[0] = linearized_x;
  trans_host[1] = trans.cast<float>();

  thrust::device_vector<Eigen::Isometry3f> trans_vec(2);
  thrust::copy(trans_host.begin(), trans_host.end(), trans_vec.begin());

  return compute_derivatives(*source_points, *source_covariances, *voxelmap, *voxel_correspondences, trans_vec.data(), trans_vec.data() + 1, H, b);
}
