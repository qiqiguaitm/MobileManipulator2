...
struct voxel_bucket_assignment_kernel {
  voxel_bucket_assignment_kernel(
    ...
    thrust::device_vector<IntPair>& index_buckets,
  ...
  __device__ void operator()(int point_index) const {
    ...
    for (int i = 0; i < info.max_bucket_scan_count; i++) {
      ...
      IntPair* index_bucket = thrust::raw_pointer_cast(index_buckets_ptr) + bucket_index;
  ...
  thrust::device_ptr<IntPair> index_buckets_ptr;
...
struct voxel_coord_select_kernel {
  ...
  __device__ VoxelBucket operator()(const IntPair& index_bucket) const {
    if (index_bucket.first < 0) {
      return VoxelBucket(Eigen::Vector3i(0, 0, 0), -1);
    }

    return VoxelBucket(thrust::raw_pointer_cast(point_coords_ptr)[index_bucket.first], index_bucket.second);
  }
...
struct accumulate_points_kernel {
  accumulate_points_kernel(
    ...
    const thrust::device_vector<VoxelBucket>& buckets,
...
GaussianVoxelMap::GaussianVoxelMap(float resolution, int init_num_buckets, int max_bucket_scan_count) : init_num_buckets(init_num_buckets) {
  ...
  thrust::host_vector<VoxelMapInfo> h_info(1);
  h_info[0] = voxelmap_info;
  thrust::copy(h_info.begin(), h_info.end(), voxelmap_info_ptr.begin());
}
...
void GaussianVoxelMap::create_bucket_table(cudaStream_t stream, const thrust::device_vector<Eigen::Vector3f>& points) {
  ...
  for (int num_buckets = init_num_buckets; init_num_buckets * 4; num_buckets *= 2) {
    ...
    // Fill with IntPair(-1, -1)
    thrust::host_vector<IntPair> h_init(num_buckets, IntPair(-1, -1));
    thrust::copy(h_init.begin(), h_init.end(), index_buckets.begin());
