...
// Forward declarations for thrust (CUDA 12.x compatible)
namespace thrust {
template <typename T>
...
class NDTCudaCore {
public:
  // Use custom Pair to avoid thrust::pair ambiguity in CUDA 12.x
  using Points = thrust::device_vector<Eigen::Vector3f>;
  using Indices = thrust::device_vector<int>;
  using Matrices = thrust::device_vector<Eigen::Matrix3f>;
  using Correspondences = thrust::device_vector<IntPair>;
  using VoxelCoordinates = thrust::device_vector<Eigen::Vector3i>;
