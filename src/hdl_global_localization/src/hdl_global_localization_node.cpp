...
public:
  GlobalLocalizationNode(const rclcpp::NodeOptions& options) : rclcpp::Node("global_localization_node", options) {
    declare_parameter<double>("globalmap_downsample_resolution", 0.2);
    declare_parameter<double>("query_downsample_resolution", 0.2);
    globalmap_downsample_resolution = get_parameter("globalmap_downsample_resolution").get_value<double>();
    query_downsample_resolution = get_parameter("query_downsample_resolution").get_value<double>();
...
private:
  ...
  bool set_global_map(srv::SetGlobalMap::Request::SharedPtr req, srv::SetGlobalMap::Response::SharedPtr res) {
    ...
    size_t num_before = cloud->size();

    cloud = downsample(cloud, globalmap_downsample_resolution);
    size_t num_after = cloud->size();

    RCLCPP_INFO_STREAM(get_logger(), "Global Map Received: " << num_before << " points, after downsample("
      << globalmap_downsample_resolution << "m): " << num_after << " points");

    if (num_after < 128) {
      RCLCPP_WARN_STREAM(get_logger(), "Downsampled map has too few points (" << num_after
        << "). Consider lowering globalmap_downsample_resolution (current: " << globalmap_downsample_resolution << ")");
    }
