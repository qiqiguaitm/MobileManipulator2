...
class GraspAnythingOnline:
    ...
    def forward(self, rgb, depth=None, **kwargs):
        ...
        assert bs == 1, f'{bs=}'
        obj_num = len(objs[0])
        #print(f"[GraspAPI] Detected {obj_num} objects")
        if obj_num == 0:
    ...
    def post_process(self, ret, rgb, depth, **kwargs):
        ...
        根据检测的物体掉码和抓取框，计算实际的抓取接触点。
        通过线段与掉码的交集计算，获取最优的夹爪放置位置。
        ...
        Returns:
            更新后的检测结果，包含接触点信息
        """
        for obj in ret[0]:  # 遍历每个检测到的物体
            if 'dt_mask' in obj:  # 如果存在实例分割掉码
                m = obj['dt_mask']  # 物体掉码
                affs = obj['affs']  # 抓取点列表
                touching_points = []  # 接触点列表
                
                for rb in affs:  # 遍历每个抓取点
                    # breakpoint()
                    xc, yc, w, h, angle2 = rb
...
class DepthOptimizerOnline:
    ...
    def forward(self, rgb, depth, chosen_policy=None, **kwargs):
        """前向推理函数，调用深度去噪服务
